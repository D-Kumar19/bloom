...
const PREVIEW_FROM = 'your favorite softie'
      ...
      className={`relative rounded-2xl border-2 p-2 text-left transition hover:-translate-y-0.5 hover:shadow-lg ${surprising ? 'cursor-wait opacity-90' : 'cursor-pointer'} ${
        isActive
          ...
          : isSurpriseHighlight
            ...
            : 'border-bloom-rose/15 bg-surface hover:border-bloom-rose/40'
      } ${className}`}
    >
      {isActive ? (
        <span
      ...
      <p className="relative z-10 mt-2 px-2 text-sm font-medium text-bloom-ink">{name}</p>
      <p className="relative z-10 px-2 pb-2 text-xs italic text-bloom-ink">{tagline}</p>
    </article>
  )
}

export function CardStylePicker({ selectedId, onSelect }: CardStylePickerProps) {
    ...
    <div className="mx-auto w-full max-w-6xl">
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <button
          ...
          className="rounded-full border border-brand-pink/30 bg-surface px-4 py-2 text-sm font-semibold text-brand-pink shadow-sm transition hover:border-brand-pink hover:bg-brand-pink/5 disabled:opacity-60"
        >
          {surprising ? 'Picking...' : '✨ Surprise me'}
        </button>
        <p className="text-xs text-bloom-ink">Can&apos;t decide? Let Bloom pick for you.</p>
      </div>

      {selected ? (
        <p className="mb-4 text-sm text-bloom-ink">
          <span className="font-semibold">{selected.name} selected</span>
          <span className="mx-2 text-bloom-ink/40">·</span>
          <span className="italic">{selected.tagline}</span>
        </p>
      ) : null}

      <div className="mb-5">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-bloom-ink">
